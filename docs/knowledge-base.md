...
- **Subscriptions are rate-capped at the transport, before decode.**
  ...
  app has to be read with that in mind. Foxglove applies it; rosbridge does not.
- **Under Xvfb, CPU tracks the pane's redraw rate, not the bandwidth.**
  9.6 MB/s at 10 Hz costs less than 697 B/s at 58 Hz. That column is llvmpipe
  drawing the window in software, and a GPU should collapse most of it — so
  treat headless CPU numbers as a rendering measurement first.
- **CDR aligns every four-byte primitive to four bytes relative to the body,**
  including the length prefix of a string or array. Getting this wrong in the
  bench bridge made the decoder read `"jpeg"` as a length and ask for 1.7 GB.
...
1. **Incoming frames queue without a bound.** At 178 MiB/s (1080p rgb8, 30 Hz)
   the app runs **2.8 seconds behind** at 14.8 Hz and holds **5.5 GB** resident.
   The backlog is the memory. The Tauri app is worse (7.0 GB), which is no
   defence: a client seconds behind and growing has already failed. See
   `docs/benchmarks.md`.
2. **The rosbridge transport ignores the subscription rate cap.** Foxglove
   enforces `default_target_hz_for_schema` via `min_interval_ns`; rosbridge does
   not, so a 100 Hz topic displays at 98.8 Hz there and 58 Hz over Foxglove. One
   policy, applied by one of two transports.
3. **The browser build has never been seen running.** It compiles again (it did
...
4. **This app burns ~0.7% CPU drawing nothing.** Found by benchmarking, not by
